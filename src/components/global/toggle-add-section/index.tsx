...
const ToggleAddSection = (props: Props) => {
  const [isAddingSection, setIsAddingSection] = useState(false);
  return (
    <>
      {isAddingSection ? (
        <SectionForm onCancel={() => setIsAddingSection(false)} />
      ) : (
        <div
          onClick={() => setIsAddingSection(true)}
          className="relative transition-all duration-300 opacity-0 hover:opacity-100"
        >
